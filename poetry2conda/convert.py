...
from typing import Mapping, TextIO, Tuple, Iterable, Optional, List
...
def convert(
    ...
    conda_only_dependencies = poetry2conda_config.get("condaonly", {})

    channels = poetry2conda_config.get("channels", None)

    dependencies, pip_dependencies = collect_dependencies(
        poetry_dependencies, conda_constraints, conda_only_dependencies
    )
    conda_yaml = to_yaml_string(env_name, dependencies, pip_dependencies, channels)
    return conda_yaml
...
def collect_dependencies(
    poetry_dependencies: Mapping, conda_constraints: Mapping, conda_only_dependencies: Mapping
    ...
    conda_only_dependencies
        A dictionary with conda only dependencies as declared with poetry2conda.
        ...
        if isinstance(constraint, str):
            ...
            if "git" in constraint:
                ...
                if "tag" in constraint:
                    tag = constraint["tag"]
                elif "branch" in constraint:
                    tag = constraint["branch"]
                elif "rev" in constraint:
                    tag = constraint["rev"]
                else:
                    raise ValueError("Git repository must specify `tag`, `branch` or `rev`")
    ...
    # 3. Add conda only dependencies
    for name, constraint in conda_only_dependencies.items():
        if isinstance(constraint, str):
            dependencies[name] = convert_version(constraint)
        elif isinstance(constraint, dict):
            assert "version" in constraint, "`version` must be provided for condaonly deps"
            dependencies[name] = convert_version(constraint["version"])
            if "channel" in constraint:
...
def to_yaml_string(
    env_name: str, dependencies: Mapping, pip_dependencies: Mapping, channels: Optional[List[str]] = None
) -> str:
