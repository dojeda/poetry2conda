...
gitgit = { git = "https://github.com/gitgit/repo.git", branch = "master" }
...
dependencies:
  ...
  - pip:
    - baz>=0.4.5,<0.5.0
    - git+https://github.com/organization/repo.git@v2.7.4#egg=grault
    - git+https://github.com/gitgit/repo.git@master#egg=gitgit
"""
...
def test_simple_sample(tmpdir, mocker):
    toml_file = tmpdir / "pyproject.toml"
    yaml_file = tmpdir / "environment.yaml"

    with toml_file.open("w") as fd:
        fd.write(SAMPLE_SIMPLE_TOML)
    expected = yaml.safe_load(io.StringIO(SAMPLE_SIMPLE_YAML))
